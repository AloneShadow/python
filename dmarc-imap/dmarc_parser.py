...
def from_spam_to_incoming():
 """ Move from Spam folder to Incoming folder. """
 box.select('Spam')
 status, msg_set = box.search(None, 'UNSEEN') #ищем письма
 print (status, "Total count of messages in Spam is %s" % len(msg_set[0].split()))
 while msg_set[0]:
    ...
    for i in msg_set[0].split():
        box.copy(i.decode(), "Inbox")
    status, msg_set = box.search(None, 'UNSEEN') # ищем письма
    print ("Total count of messages in Spam is %s" % len(msg_set[0].split()))
 box.close()
...
def find_hostname():
    ...
    SELECT_REPORT = "SELECT DISTINCT ip FROM report WHERE hostname=''"
    UPDATE_REPORT = "UPDATE report SET hostname=%s where ip=%s"
    cursor.execute(SELECT_REPORT)
    for ip in cursor.fetchall():
      ip = ip[0]
      if ':' in ip: continue
      try:
        hostname = subprocess.check_output(["host", ip]).split()[-1]
      except:
        hostname = b"Absent"
      cursor.execute(UPDATE_REPORT, (hostname,ip))
      db.commit()
...
#for msg in msg_set[0].split()[8].decode():
 ...
 for item in xml_doc.findall("record"):
    ...
    try: h_from = item.find("identifiers/header_from").text
    except: h_from = None
